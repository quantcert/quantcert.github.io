...
def get_coef_from_optimization_file(filename, iteration, evaluation=False):
  """ The file fed in this function must be a csv file with one of columns being
  ...
  :param str filename: Name of the CSV file containing the information about the
    Mermin coefficients.
  ...
  :returns: tuple[list[tuple[real]]], real(optional) -- The mermin coefficients 
...
def _main(l, r, nWires, optimization_filepath, local=True, simulation=True, epsilon=0.1):
  ...
  :param Path optimization_filepath: Path of the file containing the result on 
    the optimization process. This file has to be compatible with the function 
    ``get_coef_from_optimization_file``.
  :param bool local: If ``True`` the local simulator is used, otherwise, a call
    to the IBMQ API will be made.
  :param bool simulator: If ``True``, the job will be run on a simulator, be it 
    local or on IBM's servers, otherwise, it will be run on the quantum 
    processors made available by IBM.
  :param real epsilon: For now, only used with simulation, this is the precision
    parameter used as a proof of concept for RAC in python.
  :returns: None
  """
  if not local:
  ...
  for k in range(QFT_lenght(nWires)+1):
